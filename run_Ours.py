...
from net.net import PolicyNet, ValueNet, GATBlock, NextObsPredictor
...
if __name__ == '__main__':
    ...
    if len(set(state_dim)) == 1:
        ...
        args.model_name = 'Ours_GATv2_pred'
    ...
    # auxiliary next-observation predictor (hᵢᵗ → ôᵢᵗ⁺¹)         
    predictor = NextObsPredictor(
        h_dim=global_emb_dim,
        obs_dim=state_dim,
        concat_obs=True).to(device)
    ...
    params   = list(gat.parameters()) + list(predictor.parameters())
    ...
    for seed in trange(args.seed[0], args.seed[-1] + 1, mininterval=40, ncols=70):
        ...
        return_list, train_time = train_ours_agent(env, marl, agent_name, gat,predictor, optimizer, scheduler,  args.writer,
